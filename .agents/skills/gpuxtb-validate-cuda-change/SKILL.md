---
name: gpuxtb-validate-cuda-change
description: Validate gpuxtb CUDA kernels and public runtime behavior on a real NVIDIA GPU, including CPU parity, host/device/mixed descriptors, ragged failures, streams, Graphs, cache and WARM epochs, publication, installed consumers, sanitizers, and steady-state constraints. Use for CUDA source, CUDA-visible ABI, runtime staging, cache, SCC, force, performance, or release-gate changes.
---

# Validate a gpuxtb CUDA Change
...
3. Run public conformance for every affected memory mode with `tools/conformance/gpuxtb_public_api.py`.
4. Run `tools/conformance/gpuxtb_invariants.py` for affected modes.
