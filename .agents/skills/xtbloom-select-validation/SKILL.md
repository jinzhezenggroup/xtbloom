---
name: xtbloom-select-validation
description: Select and execute xTBloom validation from the changed files and acceptance criteria, including CPU, CUDA, Python, ABI, packaging, generated-data, licensing, and benchmark gates. Use for any material xTBloom code, test, build, packaging, or data change, especially before handoff, review, or merge when a green but incomplete test configuration would be dangerous.
---

# Select xTBloom Validation
...
| Public header, validation, symbols, install API | Invoke `$xtbloom-evolve-c-abi`; add shared/static installs and external consumers |
| GFN2 physics, SCC, energy, force, charge, QM/MM, parameters, or oracle behavior | Invoke `$xtbloom-prove-scientific-change`; require independent evidence in addition to self-consistency |
| CUDA source or CUDA runtime behavior | Invoke `$xtbloom-validate-cuda-change`; use explicit CUDA `ON` and a real NVIDIA GPU |
...
| License, dependency, install, sdist, or wheel payload | Invoke `$xtbloom-audit-dependency`; test every changed artifact boundary |
| Benchmark harness or performance claim | Run benchmark self-tests and invoke `$xtbloom-record-performance` for evidence |
...
For a public CPU gate, configure a separate shared build with `XTBLOOM_CPU_LINALG_LIBRARY` set to an absolute compatible LP64 runtime. Do not infer full inference coverage from the smaller core-only build.
...
For a public shared CPU configuration, the registration list should include the relevant names from this set: `xtbloom.c_api`, `xtbloom.batch_validation`, `xtbloom.runtime`, `xtbloom.cpu.public_inference`, `xtbloom.gfn2.eigensolver`, `xtbloom.conformance.public_cpu`, and `xtbloom.conformance.invariants_cpu`. Shared UNIX builds add `xtbloom.abi_symbols` and CUDA dependency tests. The exact list is configuration-dependent; an absent name is evidence to explain, not permission to claim coverage.
...
- `XTBLOOM_ENABLE_CUDA=AUTO` does not prove CUDA was compiled.
