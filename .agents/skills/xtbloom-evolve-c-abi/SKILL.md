---
name: xtbloom-evolve-c-abi
description: Implement and review changes to xTBloom's stable public C ABI across headers, initialization, validation, CPU/CUDA execution, Python ctypes, symbols, packaging, and install consumers. Use whenever changing `include/xtbloom/xtbloom.h`, public structures or tags, `xtbloom_*` symbols, descriptors, result semantics, ABI suffixes, or their Python mirror.
---

# Evolve the xTBloom C ABI

Preserve existing callers while carrying one public contract through every language and backend boundary. Read `AGENTS.md`, `include/xtbloom/xtbloom.h`, `src/api.cpp`, and both runtime validation layers before editing.
...
```bash
rg -n 'struct_size|api_version|_V[0-9]+_SIZE|xtbloom_.*_init' \
