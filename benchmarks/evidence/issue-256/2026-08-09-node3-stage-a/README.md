...
- Threads: 16 gpuxtb context workers. The benchmark process had no
...
every raw sample against `gpuxtb-fresh.json` under the live cross-engine gates.
...
17-iteration gpuxtb FRESH solve with a two-iteration persistent tblite solve;
...
python3 benchmarks/natoms_scaling.py \
  --engine gpuxtb --library "$PWD/build/batch1-cpu/libgpuxtb.so.0.1.0" \
  ...
  --output-json build/benchmarks/issue-256-stage-a-final/gpuxtb-fresh.json \
  --output-csv build/benchmarks/issue-256-stage-a-final/gpuxtb-fresh.csv
...
python3 benchmarks/natoms_scaling.py \
  ...
  --energy-reference-json \
    build/benchmarks/issue-256-stage-a-final/gpuxtb-fresh.json \
  --cross-engine-energy-atol 5e-7 --cross-engine-force-atol 5e-6 \
  --output-json build/benchmarks/issue-256-stage-a-final/gpuxtb-warm.json \
  --output-csv build/benchmarks/issue-256-stage-a-final/gpuxtb-warm.csv
```
...
```bash
GPUXTB_PHASE_TRACE=1 python3 benchmarks/natoms_scaling.py \
  --engine gpuxtb --library build/phase-trace/libgpuxtb.so.0.1.0 \
  --backend cpu --cpu-threads 16 --property force \
