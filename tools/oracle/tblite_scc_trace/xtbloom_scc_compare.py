"""Compare ``xtbloom-scc-trace-v1`` traces and validated iteration snapshots.

This module is the comparison foundation for issue #49.  It deliberately does
not execute xtbloom, inject an SCC state, or replay a mixer history; backend
...
_MODULE_PATH = Path(__file__).resolve().parent / "xtbloom_scc_trace.py"
_SPEC = importlib.util.spec_from_file_location("xtbloom_scc_trace", _MODULE_PATH)
