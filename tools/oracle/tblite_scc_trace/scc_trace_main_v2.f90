...
!   spin_channels (must be 2 for this unrestricted v2 recorder)
!
...
program scc_trace_main_v2
   use mctc_env, only : wp
   use xtbloom_tblite_scc_trace_recorder_v2, only : play
   implicit none

   integer :: nat, i, ip, npc, spin_channels, unpaired_electrons, mixer_memory
   ...
   read(10, *) spin_channels
   close(10)

   if (spin_channels /= 2) error stop "v2 recorder requires spin_channels=2"
