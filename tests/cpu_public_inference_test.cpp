...
struct PublicBatch {
  ...
  void bind_fields(std::vector<std::uint8_t>* payload_storage,
                   std::vector<xtbloom_interaction_t>* descriptor_storage) {
    if (fields.empty()) {
      return;
    }
...
bool warm_rejection_is_atomic(xtbloom_context_t* context, PublicBatch& request) {
  const PublicOutputImage before(request);
  return xtbloom_compute(context, &request.batch, &request.options, &request.result) ==
...
ContextHandle make_cpu_context(std::int32_t cpu_threads = 0) {
  ...
  const xtbloom_status_t status = xtbloom_context_create(&options, &raw_context);
...
int test_explicit_thread_counts_are_deterministic() {
  ...
  CHECK(xtbloom_compute(serial_context.get(), &serial_single.batch, &serial_single.options,
                        &serial_single.result) == XTBLOOM_STATUS_SUCCESS);
  CHECK(xtbloom_compute(parallel_context.get(), &parallel_single.batch, &parallel_single.options,
                        &parallel_single.result) == XTBLOOM_STATUS_SUCCESS);
...
int test_concurrent_calls_on_one_context_are_serialized_transactions() {
  ...
  CHECK(xtbloom_compute(reference_context.get(), &reference_first.batch, &reference_first.options,
                        &reference_first.result) == XTBLOOM_STATUS_SUCCESS);
  CHECK(xtbloom_compute(reference_context.get(), &reference_second.batch, &reference_second.options,
                        &reference_second.result) == XTBLOOM_STATUS_SUCCESS);
  ...
  for (int repetition = 0; repetition < 8; ++repetition) {
    ...
    std::thread first([&] {
      ...
      first_status = xtbloom_compute(shared_context.get(), &actual_first.batch,
    ...
    std::thread second([&] {
      ...
      second_status = xtbloom_compute(shared_context.get(), &actual_second.batch,
...
int test_batch_matches_sequential_and_reuses_context() {
  ...
  const std::uint32_t flags =
      XTBLOOM_COMPUTE_ENERGY | XTBLOOM_COMPUTE_FORCES | XTBLOOM_COMPUTE_ATOMIC_CHARGES;
  batch.bind(flags);
  CHECK(xtbloom_compute(context.get(), &batch.batch, &batch.options, &batch.result) ==
        XTBLOOM_STATUS_SUCCESS);
  for (std::size_t system = 0; system < 2u; ++system) {
    CHECK(batch.statuses[system] == XTBLOOM_STATUS_SUCCESS);
  ...
  for (std::size_t system = 0; system < 2u; ++system) {
    ...
    CHECK(xtbloom_compute(context.get(), &sequential.batch, &sequential.options,
                          &sequential.result) == XTBLOOM_STATUS_SUCCESS);
    CHECK(sequential.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  ...
  CHECK(xtbloom_compute(context.get(), &batch.batch, &batch.options, &batch.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(batch.statuses[0] == XTBLOOM_STATUS_SUCCESS);
...
int test_worker_call_failure_is_atomic_and_recoverable() {
  ...
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(std::strstr(xtbloom_get_last_error(), "positions are too large") != nullptr);
...
int test_public_unrestricted_energy_forces() {
  ...
  CHECK(request.batch.struct_size >= XTBLOOM_BATCH_V2_SIZE);
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_SUCCESS);
  ...
  request.unpaired_electrons = {0, 1};
  request.bind(XTBLOOM_COMPUTE_ENERGY);
  request.batch.struct_size = XTBLOOM_BATCH_V1_SIZE;
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_SUCCESS);
  return 0;
}
...
int test_mixed_restricted_unrestricted_batch_matches_serial() {
  ...
  const std::uint32_t flags =
      XTBLOOM_COMPUTE_ENERGY | XTBLOOM_COMPUTE_FORCES | XTBLOOM_COMPUTE_ATOMIC_CHARGES;
  mixed.bind(flags);
  CHECK(xtbloom_compute(parallel_context.get(), &mixed.batch, &mixed.options, &mixed.result) ==
        XTBLOOM_STATUS_SUCCESS);

  for (std::size_t system = 0; system < 2u; ++system) {
    PublicBatch sequential = slice_system(mixed, system);
    sequential.bind(flags);
    CHECK(xtbloom_compute(serial_context.get(), &sequential.batch, &sequential.options,
                          &sequential.result) == XTBLOOM_STATUS_SUCCESS);
...
int test_unrestricted_alias_validation_is_atomic_and_recoverable() {
  ...
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(std::strstr(xtbloom_get_last_error(), "aliases") != nullptr);
  ...
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(std::strstr(xtbloom_get_last_error(), "spin_channels aliases scc_iterations") != nullptr);
  CHECK(request.positions == positions_before);
  CHECK(request.spin_channels == spin_channels_before);
  CHECK(request.energies == energies_before);
  CHECK(request.forces == forces_before);
...
int test_oh_radical_unrestricted_force_matches_energy_finite_difference() {
  ...
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(request.statuses == std::vector<std::int32_t>({XTBLOOM_STATUS_SUCCESS}));
  ...
  request.positions[3] += step;
  request.bind(XTBLOOM_COMPUTE_ENERGY | XTBLOOM_COMPUTE_FORCES);
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_SUCCESS);
  for (std::size_t axis = 0u; axis < 3u; ++axis) {
...
int test_point_charges_and_periodic_operator() {
  ...
  CHECK(std::all_of(request.point_forces.begin(), request.point_forces.end(),
                    [](double value) { return std::isfinite(value); }));
  CHECK((request.result.flags & XTBLOOM_RESULT_FORCES_EXCLUDE_EXTERNAL_OPERATOR_DERIVATIVES) != 0u);

  request.bind(XTBLOOM_COMPUTE_POINT_CHARGE_FORCES);
...
int test_electric_field_golden_energy_and_scc_state() {
  ...
  request.unpaired_electrons = {0};
  const std::uint32_t flags = XTBLOOM_COMPUTE_ENERGY | XTBLOOM_COMPUTE_FORCES |
                              XTBLOOM_COMPUTE_ATOMIC_CHARGES | XTBLOOM_COMPUTE_DIPOLE_MOMENTS;
  request.bind(flags);
  CHECK(xtbloom_compute(context.get(), &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(request.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  ...
  CHECK((request.result.flags & XTBLOOM_RESULT_DIPOLE_MOMENTS) != 0u);
...
int test_electric_field_warm_identity_is_strict() {
  ...
  CHECK(xtbloom_compute(context.get(), &h2.batch, &h2.options, &h2.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(h2.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  ...
  h2.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(warm_rejection_is_atomic(context.get(), h2));
  CHECK(std::strstr(xtbloom_get_last_error(), "identical") != nullptr);
  ...
  CHECK(xtbloom_compute(zero_context.get(), &explicit_zero.batch, &explicit_zero.options,
                        &explicit_zero.result) == XTBLOOM_STATUS_SUCCESS);
  CHECK(explicit_zero.statuses[0] == XTBLOOM_STATUS_SUCCESS);

  explicit_zero.fields.clear();
  explicit_zero.bind(flags);
  explicit_zero.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
...
int test_public_warm_start_transactions() {
  ...
  perturbed.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  CHECK(xtbloom_compute(fresh_reference.get(), &reference.batch, &reference.options,
  ...
  perturbed.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(xtbloom_compute(context.get(), &perturbed.batch, &perturbed.options, &perturbed.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(perturbed.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  ...
  changed_flags.bind(XTBLOOM_COMPUTE_ENERGY);
  changed_flags.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  changed_charge.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  changed_unpaired.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  changed_spin.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  changed_iterations.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  changed_charge_tolerance.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  changed_energy_tolerance.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  CHECK(xtbloom_compute(context.get(), &unsupported.batch, &unsupported.options,
                        &unsupported.result) == XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(std::strstr(xtbloom_get_last_error(), "unsupported atomic number") != nullptr);
  ...
  methane.bind(flags);
  methane.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(warm_rejection_is_atomic(context.get(), methane));

  methane.bind(flags);
  CHECK(xtbloom_compute(context.get(), &methane.batch, &methane.options, &methane.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(methane.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  methane.bind(flags);
  methane.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(xtbloom_compute(context.get(), &methane.batch, &methane.options, &methane.result) ==
        XTBLOOM_STATUS_SUCCESS);
  ...
  CHECK(xtbloom_compute(nonconverged_context.get(), &forced.batch, &forced.options,
                        &forced.result) == XTBLOOM_STATUS_SUCCESS);
  CHECK(forced.statuses[0] == XTBLOOM_STATUS_SCC_NOT_CONVERGED);
  CHECK(forced.converged[0] == 0u);
  CHECK(std::isnan(forced.energies[0]));
  forced.bind(flags);
  forced.options.max_scc_iterations = 1;
  forced.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  mixed.spin_channels = {1, 2};
  mixed.bind(flags);
  CHECK(xtbloom_compute(mixed_context.get(), &mixed.batch, &mixed.options, &mixed.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(mixed.statuses ==
        std::vector<std::int32_t>({XTBLOOM_STATUS_SUCCESS, XTBLOOM_STATUS_SUCCESS}));
  mixed.bind(flags);
  mixed.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(xtbloom_compute(mixed_context.get(), &mixed.batch, &mixed.options, &mixed.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(mixed.statuses ==
        std::vector<std::int32_t>({XTBLOOM_STATUS_SUCCESS, XTBLOOM_STATUS_SUCCESS}));
  ...
  const std::uint32_t embedded_flags = flags | XTBLOOM_COMPUTE_POINT_CHARGE_FORCES;
  embedded.bind(embedded_flags);
  CHECK(xtbloom_compute(embedded_context.get(), &embedded.batch, &embedded.options,
                        &embedded.result) == XTBLOOM_STATUS_SUCCESS);
  ...
  embedded.bind(embedded_flags);
  embedded.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(xtbloom_compute(embedded_context.get(), &embedded.batch, &embedded.options,
                        &embedded.result) == XTBLOOM_STATUS_SUCCESS);
  CHECK((embedded.result.flags & XTBLOOM_RESULT_FORCES_EXCLUDE_EXTERNAL_OPERATOR_DERIVATIVES) !=
        0u);
  ...
  changed_points.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  disabled_periodic.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  ...
  peers.bind(flags);
  CHECK(xtbloom_compute(peer_context.get(), &peers.batch, &peers.options, &peers.result) ==
        XTBLOOM_STATUS_SUCCESS);
  peers.positions[9] = peers.positions[6] + 1.1e-6;
  peers.bind(flags);
  peers.options.scc_start_mode = XTBLOOM_SCC_START_WARM;
  CHECK(xtbloom_compute(peer_context.get(), &peers.batch, &peers.options, &peers.result) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(peers.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  CHECK(peers.statuses[1] == XTBLOOM_STATUS_EIGENSOLVER_FAILED);
...
int test_degenerate_occupation_representability_is_publicly_successful() {
  ...
  CHECK(request.converged[0] == 1u);
  CHECK(std::isfinite(request.energies[0]));
  CHECK(request.statuses[1] == XTBLOOM_STATUS_SUCCESS);
...
int test_plan_creation_model_and_abi_prefix_contracts() {
  ...
  xtbloom_plan_t* raw_short_plan = nullptr;
  CHECK(xtbloom_plan_create(context.get(), &request.batch, short_options, &raw_short_plan) ==
        XTBLOOM_STATUS_SUCCESS);
...
int test_plan_create_query_workspace_and_reuse() {
  ...
  xtbloom_workspace_query_t invalid_query = query;
  invalid_query.compute_flags = 0u;
  invalid_query.host_required_bytes = UINT64_C(0x1122334455667788);
  CHECK(xtbloom_plan_query_workspace(plan.get(), &invalid_query) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  ...
  CHECK(xtbloom_plan_query_workspace(plan.get(), &invalid_query) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  invalid_query = query;
  invalid_query.compute_flags = XTBLOOM_COMPUTE_ENERGY;
  CHECK(xtbloom_plan_query_workspace(plan.get(), &invalid_query) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  ...
  energy_request.bind(XTBLOOM_COMPUTE_ENERGY);
  xtbloom_plan_t* raw_energy_plan = nullptr;
  CHECK(xtbloom_plan_create(context.get(), &energy_request.batch, &energy_request.options,
  ...
  xtbloom_plan_t* raw_explicit_offsets_plan = nullptr;
  CHECK(xtbloom_plan_create(context.get(), &explicit_zero_point_offsets.batch,
                            &explicit_zero_point_offsets.options,
                            &raw_explicit_offsets_plan) == XTBLOOM_STATUS_SUCCESS);
  PlanHandle explicit_offsets_plan(raw_explicit_offsets_plan);
  xtbloom_workspace_query_t explicit_offsets_query{};
  CHECK(xtbloom_workspace_query_init(&explicit_offsets_query, sizeof(explicit_offsets_query)) ==
        XTBLOOM_STATUS_SUCCESS);
  explicit_offsets_query.compute_flags = flags;
  CHECK(xtbloom_plan_query_workspace(explicit_offsets_plan.get(), &explicit_offsets_query) ==
        XTBLOOM_STATUS_SUCCESS);
  ...
  xtbloom_plan_t* second_plan = nullptr;
  CHECK(xtbloom_plan_create(context.get(), &request.batch, &request.options, &second_plan) ==
        XTBLOOM_STATUS_SUCCESS);
  CHECK(second_plan != nullptr);
  xtbloom_plan_destroy(second_plan);

  /* Plan compute reproduces the exact convenience-path results. */
  PublicBatch via_compute = request;
  via_compute.bind(flags);
  CHECK(xtbloom_compute(context.get(), &via_compute.batch, &via_compute.options,
                        &via_compute.result) == XTBLOOM_STATUS_SUCCESS);
...
int test_plan_fixed_topology_zero_steady_state_allocations() {
  ...
  other.bind(flags);
  xtbloom_plan_t* raw_other_plan = nullptr;
  CHECK(xtbloom_plan_create(context.get(), &other.batch, &other.options, &raw_other_plan) ==
        XTBLOOM_STATUS_SUCCESS);
  ...
  allocation_test::enabled.store(true, std::memory_order_release);
  const xtbloom_status_t status =
      xtbloom_plan_compute(plan.get(), &request.batch, &request.options, &request.result);
  allocation_test::enabled.store(false, std::memory_order_release);
  CHECK(status == XTBLOOM_STATUS_SUCCESS);
  CHECK(allocation_test::count.load(std::memory_order_relaxed) == 0u);
  CHECK(request.statuses[0] == XTBLOOM_STATUS_SUCCESS);
  ...
  xtbloom_workspace_query_t workspace_before{};
  CHECK(xtbloom_workspace_query_init(&workspace_before, sizeof(workspace_before)) ==
        XTBLOOM_STATUS_SUCCESS);
  workspace_before.compute_flags = flags;
  CHECK(xtbloom_plan_query_workspace(plan.get(), &workspace_before) == XTBLOOM_STATUS_SUCCESS);
  ...
  const xtbloom_status_t field_status =
      xtbloom_plan_compute(plan.get(), &request.batch, &request.options, &request.result);
  allocation_test::enabled.store(false, std::memory_order_release);
  CHECK(field_status == XTBLOOM_STATUS_SUCCESS);
  CHECK(allocation_test::count.load(std::memory_order_relaxed) == 0u);
  ...
  std::vector<xtbloom_interaction_t> reference_descriptors;
  reference.bind(flags);
  reference.bind_fields(&reference_payload, &reference_descriptors);
  CHECK(xtbloom_compute(reference_context.get(), &reference.batch, &reference.options,
                        &reference.result) == XTBLOOM_STATUS_SUCCESS);
  ...
  allocation_test::count.store(0u, std::memory_order_relaxed);
  allocation_test::enabled.store(true, std::memory_order_release);
  const xtbloom_status_t changed_field_status =
      xtbloom_plan_compute(plan.get(), &request.batch, &request.options, &request.result);
  allocation_test::enabled.store(false, std::memory_order_release);
  CHECK(changed_field_status == XTBLOOM_STATUS_SUCCESS);
  CHECK(allocation_test::count.load(std::memory_order_relaxed) == 0u);
  ...
  allocation_test::count.store(0u, std::memory_order_relaxed);
  allocation_test::enabled.store(true, std::memory_order_release);
  const xtbloom_status_t detached_field_status =
      xtbloom_plan_compute(plan.get(), &request.batch, &request.options, &request.result);
  allocation_test::enabled.store(false, std::memory_order_release);
  CHECK(detached_field_status == XTBLOOM_STATUS_SUCCESS);
  CHECK(allocation_test::count.load(std::memory_order_relaxed) == 0u);
  CHECK(request.energies == no_field_energies);
  CHECK(request.forces == no_field_forces);

  xtbloom_workspace_query_t workspace_after{};
  CHECK(xtbloom_workspace_query_init(&workspace_after, sizeof(workspace_after)) ==
        XTBLOOM_STATUS_SUCCESS);
  workspace_after.compute_flags = flags;
  CHECK(xtbloom_plan_query_workspace(plan.get(), &workspace_after) == XTBLOOM_STATUS_SUCCESS);
...
int test_plan_topology_mismatch_fails_before_output_mutation() {
  ...
  PublicBatch request = make_repeated_h2_he_batch(2u);
  request.bind(flags);
  xtbloom_plan_t* null_batch_plan = reinterpret_cast<xtbloom_plan_t*>(UINTPTR_MAX);
  CHECK(xtbloom_plan_create(context.get(), nullptr, &request.options, &null_batch_plan) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(null_batch_plan == nullptr);
  xtbloom_plan_t* null_options_plan = reinterpret_cast<xtbloom_plan_t*>(UINTPTR_MAX);
  CHECK(xtbloom_plan_create(context.get(), &request.batch, nullptr, &null_options_plan) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  ...
  CHECK(xtbloom_plan_compute(plan.get(), &other.batch, &other.options, &other.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  ...
  CHECK(xtbloom_plan_compute(nullptr, &request.batch, &request.options, &request.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(request.result.flags == UINT32_C(0x12345678));
  CHECK(xtbloom_plan_compute(plan.get(), nullptr, &request.options, &request.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(request.result.flags == UINT32_C(0x12345678));
  CHECK(xtbloom_plan_compute(plan.get(), &request.batch, nullptr, &request.result) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);
  CHECK(request.result.flags == UINT32_C(0x12345678));
  CHECK(xtbloom_plan_compute(plan.get(), &request.batch, &request.options, nullptr) ==
        XTBLOOM_STATUS_INVALID_ARGUMENT);

  PublicBatch mismatched_policy = request;
  mismatched_policy.bind(XTBLOOM_COMPUTE_ENERGY);
...
int test_plan_multi_threaded_reuse() {
  ...
  std::vector<xtbloom_status_t> statuses(workers, XTBLOOM_STATUS_INTERNAL_ERROR);
  ...
  for (std::size_t worker = 0u; worker < workers; ++worker) {
    threads.emplace_back([&, worker] {
      ...
      batch.bind(flags);
      statuses[worker] =
          xtbloom_plan_compute(plan.get(), &batch.batch, &batch.options, &batch.result);
      if (statuses[worker] != XTBLOOM_STATUS_SUCCESS) {
        std::cerr << "worker " << worker << " failed: " << xtbloom_get_last_error() << '\n';
      }
    });
  }
  ...
  for (xtbloom_status_t status : statuses) {
    if (status != XTBLOOM_STATUS_SUCCESS) {
