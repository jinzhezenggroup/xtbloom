...
geometry optimization, molecular dynamics, solvation, and native/analytic
Hessians are not implemented. Python `Calculator.hessian()` and
`BatchCalculator.hessian()` provide dense numerical QM-coordinate Hessians from
batched analytic-force differences, and the Python
[vibrational analysis](vibrations.md) post-processes those Hessians. These
features and the browser/dpdata optimizers are higher-level adapters built on
repeated xTBloom calculations.
