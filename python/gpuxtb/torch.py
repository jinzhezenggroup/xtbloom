"""PyTorch autograd integration for gpuxtb through the DLPack bridge.
    ...
    class _GPUxtbTorchFunction(torch.autograd.Function):
        ...
        def forward(
            ...
            if positions.dtype != torch.float64:
                raise GPUxtbValueError(
                    f"gpuxtb_torch requires float64 positions, got {positions.dtype}"
                )
...
def gpuxtb_torch(
    ...
    *,
    ...
    :func:`gpuxtb.compute_arrays`; ``positions`` is the only differentiable
    ...
    gpuxtb (Hartree and Hartree/bohr).
