...
xTBloom consumes arrays only through the DLPack producer protocol (see
:mod:`xtbloom._dlpack`).  This module uses ``array_api_compat`` -- a small,
...
PyTorch package is imported by xTBloom runtime code: the shims import a
...
def _compat() -> ModuleType:
    ...
    try:
        ...
        raise XTBloomValueError(
            "xTBloom requires the array_api_compat package to consume Array API arrays"
        ) from None
    return compat
