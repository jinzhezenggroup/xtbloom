"""Minimal DLPack consumer bridge for the xTBloom Python interface.
...
into a validated ``xtbloom`` descriptor view without importing any backend
...
* The producer object and the consumed capsule are kept alive through the
  caller's synchronous ``xtbloom_compute`` return.  A consumed capsule is
...
library; :mod:`xtbloom._array` owns the backend-neutral probing and this module
owns only the raw capsule contract.

No public C ABI is used here and nothing is exported from the package: this
module is an implementation detail of :class:`xtbloom.interface.ArrayBatch`.
"""
...
# DLPack stream convention: when xTBloom's context uses the native legacy CUDA
...
# ``xtbloom_buffer_t`` stores byte sizes as ``size_t`` and native validation
...
class DLPackView:
    ...
    def descriptor(self) -> library.ConstBuffer:
        """The borrowed ``xtbloom_const_buffer_t`` for this view."""
    ...
    def as_output_buffer(self) -> library.Buffer:
        """Return the writable ``xtbloom_buffer_t`` for this ``out=`` view."""
...
def memory_space_for_device(device_type: int) -> int | None:
    """Map a DLPack device kind to a ``xtbloom`` memory-space tag (or ``None``).

    ``None`` means the device kind is reserved by xTBloom (ROCm) or otherwise
    unsupported by the native execution path (CUDA-managed memory is rejected
    because xTBloom deliberately uses ``CudaManagedMemoryPolicy::kReject``).
    """
...
def consume_from_dlpack(
    ...
    *,
    ...
    if copy and isinstance(producer, np.ndarray):
        ...
        # ``copy=True`` themselves. NumPy is already a required xTBloom runtime
    ...
    if parsed.lanes != 1:
        raise XTBloomValueError(
            ...
            "xTBloom requires lane-1 (scalar) descriptors"
        )
...
def _stream_argument(stream: int | None, device_type: int) -> int | None:
    """Translate the xTBloom context stream into the DLPack stream argument.
...
class _ResultArena:
    ...
    def handle(self) -> object:
        """The native ``xtbloom_result_owner_t`` handle (kept for exports)."""
        return self._handle
    ...
    def retain(self) -> None:
        ...
        library.load_library().xtbloom_result_owner_retain(self._handle)

    def release(self) -> None:
        """Release one arena reference held by one producer slice."""
        library.load_library().xtbloom_result_owner_release(self._handle)
...
def _release_result_owner(library_instance: ctypes.CDLL, handle: object) -> None:
    ...
    library_instance.xtbloom_result_owner_release(handle)


@dataclass
class DLPackResultBuffer:
    """A DLPack producer over one xTBloom-owned result arena.
    ...
    * Repeated exports are supported: every :meth:`__dlpack__` call creates a
    ...
    def __dlpack__(
        ...
        """Export one xTBloom-owned slice as a ``dltensor`` PyCapsule.
        ...
        if copy is True:
            raise BufferError(
                "copy=True is not supported by xTBloom-owned result producers"
            )
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise XTBloomValueError(
                "this xTBloom-owned result buffer is closed and cannot be used"
            )


def _export_native_slice(buffer: DLPackResultBuffer, versioned: bool) -> int:
    """Call ``xtbloom_result_owner_export_dltensor`` for one slice."""
    ...
    status = library_instance.xtbloom_result_owner_export_dltensor(
        buffer.arena.handle,
        ctypes.byref(view),
        int(versioned),
    ...
    if status != library.STATUS_SUCCESS:
        raise XTBloomRuntimeError(
            "xtbloom_result_owner_export_dltensor failed with "
...
def _producer_dtype_fields(dtype: np.dtype) -> tuple[int, int]:
    ...
    raise XTBloomValueError(
        f"cannot export numpy dtype {dtype} through the xTBloom DLPack producer"
    )
