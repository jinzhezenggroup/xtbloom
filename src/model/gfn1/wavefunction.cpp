...
const std::array<const WavefunctionFieldLayout*, 7> field_layouts(
...
std::array<const void*, 7> view_field_pointers(const View& view) {
...
xtbloom_status_t validate_layout_metadata(const WavefunctionLayout& layout, std::string& error) {
  ...
  for (std::size_t system = 0; system < batch_count; ++system) {
    ...
    for (std::int64_t atom = atom_begin; atom < atom_end; ++atom) {
      ...
      for (std::size_t local_shell = 0; local_shell < element->shell_count; ++local_shell) {
        ...
        if (!(shell_parameters.reference_occupation >= 0.0) ||
          ...
          error = "wavefunction reference shell occupations do not match GFN1 parameters";
          return XTBLOOM_STATUS_INVALID_ARGUMENT;
        }
      ...
      if (!(atom_reference > 0.0) || !std::isfinite(atom_reference) ||
        ...
        error = "wavefunction reference atom occupations do not match GFN1 parameters";
        return XTBLOOM_STATUS_INVALID_ARGUMENT;
      }
    ...
    const std::array<std::int64_t, 7> expected_counts{
        {spin_matrix_elements, spin_orbitals, occupation_elements, spin_matrix_elements,
         spin_shells, spin_atoms, spin_matrix_elements}};
  ...
  try {
    ...
    for (WavefunctionFieldLayout* field :
         {&created.coefficients, &created.eigenvalues, &created.occupations, &created.density,
          &created.qsh, &created.qat, &created.energy_weighted_density}) {
    ...
    for (std::int64_t atom = 0; atom < basis.total_atoms; ++atom) {
      ...
      for (std::int64_t shell = shell_begin; shell < shell_end; ++shell) {
        ...
        if (basis.shell_to_atom[shell_index] != atom ||
            ...
            !std::isfinite(shell_parameters.reference_occupation)) {
          error = "wavefunction basis shell metadata does not match GFN1 parameters";
          return XTBLOOM_STATUS_INVALID_ARGUMENT;
        }
    ...
    error = "failed to allocate GFN1 wavefunction layout metadata";
...
xtbloom_status_t make_wavefunction_warm_start_identity(const WavefunctionLayout& layout,
  ...
  try {
    ...
    created.model = XTBLOOM_MODEL_GFN1_XTB;
